...
CREATE TABLE agent (id BIGINT AUTO_INCREMENT, name VARCHAR(50), surname VARCHAR(50), vat VARCHAR(20), cf VARCHAR(20), iban VARCHAR(20), sf_guard_user_id bigint, address_id bigint, parent_id bigint, INDEX address_id_idx (address_id), INDEX sf_guard_user_id_idx (sf_guard_user_id), INDEX parent_id_idx (parent_id), PRIMARY KEY(id)) ENGINE = INNODB;
CREATE TABLE booking_version (id BIGINT, product_id bigint, date_from datetime, date_to datetime, booking_status_id bigint, created_by BIGINT NOT NULL, updated_by BIGINT NOT NULL, version BIGINT, PRIMARY KEY(id, version)) ENGINE = INNODB;
CREATE TABLE booking (id BIGINT AUTO_INCREMENT, product_id bigint, date_from datetime, date_to datetime, booking_status_id bigint, created_by BIGINT NOT NULL, updated_by BIGINT NOT NULL, version BIGINT, deleted_at DATETIME, INDEX product_id_idx (product_id), INDEX booking_status_id_idx (booking_status_id), PRIMARY KEY(id)) ENGINE = INNODB;
CREATE TABLE booking_status (id BIGINT AUTO_INCREMENT, name VARCHAR(100), PRIMARY KEY(id)) ENGINE = INNODB;
...
CREATE TABLE photos (id BIGINT AUTO_INCREMENT, title VARCHAR(255), picpath VARCHAR(255), file_name VARCHAR(255), gallery_id bigint, is_default TINYINT(1) DEFAULT '0', created_at DATETIME NOT NULL, updated_at DATETIME NOT NULL, slug VARCHAR(255), UNIQUE INDEX photos_sluggable_idx (slug), INDEX gallery_id_idx (gallery_id), PRIMARY KEY(id)) ENGINE = INNODB;
CREATE TABLE product_taggable_tag (id BIGINT, tag_id BIGINT, PRIMARY KEY(id, tag_id)) ENGINE = INNODB;
CREATE TABLE product (id BIGINT AUTO_INCREMENT, code VARCHAR(50), name VARCHAR(255), description LONGTEXT, quantity bigint, consumable TINYINT(1), price DECIMAL(5, 2) DEFAULT 0, offer_price DECIMAL(5, 2) DEFAULT 0, offer_end_date datetime, category_id bigint, company_id bigint, gallery_id bigint, created_by BIGINT NOT NULL, updated_by BIGINT NOT NULL, slug VARCHAR(255), deleted_at DATETIME, UNIQUE INDEX product_sluggable_idx (slug), INDEX gallery_id_idx (gallery_id), INDEX category_id_idx (category_id), INDEX company_id_idx (company_id), PRIMARY KEY(id)) ENGINE = INNODB;
...
ALTER TABLE agent ADD CONSTRAINT agent_parent_id_agent_id FOREIGN KEY (parent_id) REFERENCES agent(id);
ALTER TABLE agent ADD CONSTRAINT agent_address_id_address_id FOREIGN KEY (address_id) REFERENCES address(id);
ALTER TABLE booking_version ADD CONSTRAINT booking_version_id_booking_id FOREIGN KEY (id) REFERENCES booking(id) ON UPDATE CASCADE ON DELETE CASCADE;
ALTER TABLE booking ADD CONSTRAINT booking_product_id_product_id FOREIGN KEY (product_id) REFERENCES product(id);
ALTER TABLE booking ADD CONSTRAINT booking_booking_status_id_booking_status_id FOREIGN KEY (booking_status_id) REFERENCES booking_status(id);
